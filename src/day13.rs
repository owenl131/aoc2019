...
fn run_arcade(mut m: Machine, my_input: Sender<i128>, my_output: Receiver<i128>) -> i32 {
    store(&mut m, 0, 2);
    let j = run_machine(m);
...
fn main() {
    ...
    let (mut m, my_input, my_output) = new_machine(arr);
    println!("{}", run_arcade(m, my_input, my_output));
}
