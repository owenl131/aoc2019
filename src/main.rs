...
fn main() {
    ...
    run_test_quine();
    run_test_big();
    let contents = fs::read_to_string("input21.txt")
        .expect("File reading failed");
    let arr = parse_input(contents.trim());

    // J = (!A || !B || !C) && ((D && H) || (D && E && I) || (D && E && F))
    // (!A || !B || !C) == !(A && B && C)
    // Hole in one of the spots which you can jump over

    // But can you land? D must be land
    // And E must be land or I must be land
    let mut program: String = "NOT A J
NOT B T
OR T J
NOT C T
OR T J
